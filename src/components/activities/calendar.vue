<template>
  <v-row class="fill-height pb-12 pt-2 mx-1">
    ...
    <v-dialog v-model="dialog" max-width="700px">
      ...
      <input-form v-bind:company="newActivityClient" v-bind:activity="editedItem" @closeDialog="closeDialog"/>
    </v-dialog>
  ...
  export default {
    ...
    methods: {
      ...
      updateRange ({ start, end }) {
        ...
        this.end = end.date
        var link = process.env.VUE_APP_BACKEND_ROUTE + "api/v2/calendars?filter[date_between]=" + this.start + ',' + this.end + "&filter[user_id]="+this.currentUser.id
      },
      mapCalendar(item){
        return item.map(id=>{
          return{
